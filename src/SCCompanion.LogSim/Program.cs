...
using SCCompanion.LogSim;

// SC Companion log simulator.
//
...
else
{
    ...
    Console.WriteLine($"  .\\start.ps1 -Path \"{liveDirectory}\"");
    Console.WriteLine($"  dotnet run --project src\\SCCompanion.Cli -c Release -- --path \"{liveDirectory}\"");
}
...
{
    public string InstallRoot { get; init; } = Path.Combine(Path.GetTempPath(), "SCCompanionFakeInstall");
    ...
    {
        Console.WriteLine("""
            SC Companion log simulator - generates a fake Star Citizen install.

              --install <dir>   Where to create the fake install (default: %TEMP%\SCCompanionFakeInstall)
            ...
            Examples:
              dotnet run --project src\SCCompanion.LogSim -- --backups 20
              dotnet run --project src\SCCompanion.LogSim -- --backups 20 --combat
              dotnet run --project src\SCCompanion.LogSim -- --live --speed 120

            Then point the app at the generated install:
              .\start.ps1 -Path "%TEMP%\SCCompanionFakeInstall\LIVE"
            """);
    }
}
