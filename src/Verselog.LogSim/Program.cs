...
using Verselog.LogSim;

// Verselog log simulator.
//
...
Console.WriteLine("Verselog log simulator  ·  by nekron");
Console.WriteLine();
...
else
{
    ...
    Console.WriteLine($"  .\\start.ps1 -Path \"{liveDirectory}\"");
    Console.WriteLine($"  dotnet run --project src\\Verselog.Cli -c Release -- --path \"{liveDirectory}\"");
}
...
{
    public string InstallRoot { get; init; } = Path.Combine(Path.GetTempPath(), "VerselogFakeInstall");
    ...
    {
        Console.WriteLine("""
            Verselog log simulator - generates a fake Star Citizen install.

              --install <dir>   Where to create the fake install (default: %TEMP%\VerselogFakeInstall)
            ...
            Examples:
              dotnet run --project src\Verselog.LogSim -- --backups 20
              dotnet run --project src\Verselog.LogSim -- --backups 20 --combat
              dotnet run --project src\Verselog.LogSim -- --live --speed 120

            Then point the app at the generated install:
              .\start.ps1 -Path "%TEMP%\VerselogFakeInstall\LIVE"
            """);
    }
}
